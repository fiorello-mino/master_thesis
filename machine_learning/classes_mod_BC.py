...
class AsymmetricPad2dConv(nn.Module):
    '''
    This class wraps 2D convolutions allowing for 2 different padding modes for each direction
    '''
    def __init__(
        self,
        in_channels     : int,
        ...
        groups          : int   = 1,
        bias            : bool  = True,
        padding_mode    : list[str] | str   = 'zeros',
        ...
        if isinstance(padding_mode, str):
            self.padding_mode = [padding_mode, padding_mode]
        elif len(padding_mode) == 1:
            self.padding_mode = [padding_mode[0], padding_mode[0]]
        elif len(padding_mode) == 2:
            self.padding_mode = [padding_mode[0], padding_mode[1]]
        else:
            raise ValueError(f'Provided padding mode argument(s) {padding_mode} is not valid. Aborting.')

        if not isinstance(kernel_size, int):
            raise NotImplementedError(f'We have not implemented asymmetric padding size at the moment, only asymmetric padding mode. Aborting. (Should be super easy, but we are lazy!)')

        self.pad_size = dilation*(kernel_size-1)//2
        ...
        self.conv = nn.Conv2d(
            ...
            stride          = stride,
            padding         = 0,            # important: padding will be dealt with functional.pad
            dilation        = dilation,
            groups          = groups,
            bias            = bias,
            device          = device,
        ...
        for kk, mode in enumerate(self.padding_mode):
            self.padding_mode[kk] = 'constant' if mode == 'zeros' else self.padding_mode[kk]


    def forward(self, x : torch.Tensor) -> torch.Tensor:

        x = f.pad(x, (self.pad_size, self.pad_size, 0, 0), mode = self.padding_mode[0])
        x = f.pad(x, (0, 0, self.pad_size, self.pad_size), mode = self.padding_mode[1])
...
class ConvGRU(nn.Module):
    ...
    def make_div_filters(self, x):
        ...
        grad1 = AsymmetricPad2dConv(1, 1, kernel_size=3, stride=1, padding=1, bias=False, padding_mode=self.padding_mode)
        grad2 = AsymmetricPad2dConv(1, 1, kernel_size=3, stride=1, padding=1, bias=False, padding_mode=self.padding_mode)
