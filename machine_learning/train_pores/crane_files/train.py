...
# <<< import my stuff <<<
from src.classes_mod_BC import *
from src.utils import *
from src.dataloaders_mod_BC import give_dataloaders, give_3D_dataloaders
...
def tot_loss_fn(x, y, eps, dx, coeffE, coeffG):
    grad_loss = bulk_grad_penalty(x, y, dx)
    e_loss = e_penalty(x, y, eps, dx)
    mse_loss = nn.MSELoss()(x,y)
    tot_loss = mse_loss + coeffE*e_loss + coeffG*grad_loss
    
    return tot_loss, mse_loss, e_loss, grad_loss
...
def main():
    ...
    if args.extract_param:
        ...
        loss_fn = lambda x,y: \
            ...
            #tot_loss_fn(x, y, args.eps, args.dx, args.coeffE, args.coeffG)
    # training loop
