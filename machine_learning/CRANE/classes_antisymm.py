...
class ConvGRU3D(nn.Module):
    ...
    def __init__(self, hidden_units, input_channels, hidden_channels, kernel_size, padding_mode, bias=True, divergence=True, separable=False, num_params=0, dropout=False, dropout_prob=None, output_channels=None, conservative=False): # ho aggiunto conservative=False
        '''
        Constructor method
        '''
    ...
    def make_div_filters(self, x):
        '''
        Costruisce i filtri spaziali addestrabili per la divergenza.
        Inizializzati come differenze centrate, verranno resi rigorosamente
        antisimmetrici 'on-the-fly' nel metodo divergence() per garantire
        la conservazione della massa.
        '''
        print('Constructing LEARNABLE differential operators...', end='')

        self.grad1 = nn.Conv3d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, padding_mode=self.padding_mode)
        self.grad2 = nn.Conv3d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, padding_mode=self.padding_mode)
        self.grad3 = nn.Conv3d(1, 1, kernel_size=3, stride=1, padding=1, bias=False, padding_mode=self.padding_mode)

        # Inizializzazione base: differenze centrate 1D [1, 0, -1]
        gradx = np.zeros((3, 3, 3));
        gradx[1, 1, 0] = -1.;
        gradx[1, 1, 2] = 1.
        grady = np.zeros((3, 3, 3));
        grady[1, 0, 1] = -1.;
        grady[1, 2, 1] = 1.
        gradz = np.zeros((3, 3, 3));
        gradz[0, 1, 1] = -1.;
        gradz[2, 1, 1] = 1.

        # Assegnazione ai tensori dei pesi (normalizzati rispetto al passo spaziale)
        self.grad1.weight = nn.Parameter(torch.from_numpy(gradx).float().unsqueeze(0).unsqueeze(0))
        self.grad2.weight = nn.Parameter(torch.from_numpy(grady).float().unsqueeze(0).unsqueeze(0))
        self.grad3.weight = nn.Parameter(torch.from_numpy(gradz).float().unsqueeze(0).unsqueeze(0))

        # SBLOCCO DEI GRADIENTI: La rete imparerà a smorzare l'overshoot!
        self.grad1.requires_grad = True
        self.grad2.requires_grad = True
        self.grad3.requires_grad = True

        self.grad1.to(x.device)
        self.grad2.to(x.device)
        self.grad3.to(x.device)

        print('DONE!')

    def divergence(self, x):
        '''
        Calcola la divergenza forzando l'antisimmetria sui kernel addestrabili.
        Questo garantisce la conservazione della massa locale e globale.
        '''
        Jx, Jy, Jz = torch.split(x, 1, dim=1)

        # Rimuove la media per le condizioni di bordo (Neumann)
        ...
        # Proiezione antisimmetrica (W - W^T) / 2
        # dims in flip: [4] è X, [3] è Y, [2] è Z (shape: Out, In, Z, Y, X)
        w_x = 0.5 * (self.grad1.weight - torch.flip(self.grad1.weight, dims=[4]))
        w_y = 0.5 * (self.grad2.weight - torch.flip(self.grad2.weight, dims=[3]))
        w_z = 0.5 * (self.grad3.weight - torch.flip(self.grad3.weight, dims=[2]))

        # Applica la convoluzione usando F.conv3d con i pesi forzati
        # Utilizziamo mode 'reflect' per essere coerenti con il padding del tensore
        Jx_pad = torch.nn.functional.pad(Jx, (1, 1, 1, 1, 1, 1), mode=self.padding_mode)
        Jy_pad = torch.nn.functional.pad(Jy, (1, 1, 1, 1, 1, 1), mode=self.padding_mode)
        Jz_pad = torch.nn.functional.pad(Jz, (1, 1, 1, 1, 1, 1), mode=self.padding_mode)

        gradx = torch.nn.functional.conv3d(Jx_pad, w_x)
        grady = torch.nn.functional.conv3d(Jy_pad, w_y)
        gradz = torch.nn.functional.conv3d(Jz_pad, w_z)
